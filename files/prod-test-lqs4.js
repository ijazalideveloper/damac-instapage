...
let refreshTokenExpiry = null; // Track refresh token expiry dynamically
...
const saveTokensToSession = (
  ...
  if (typeof window !== "undefined") {
    if (accessToken) {
      sessionStorage.setItem("lqsAccessToken", accessToken);
    }
    if (tokenExpiry !== undefined && tokenExpiry !== null) {
      sessionStorage.setItem("lqsTokenExpiry", tokenExpiry.toString());
    }
    if (refreshToken) {
      sessionStorage.setItem("lqsRefreshToken", refreshToken);
    }
    if (refreshTokenExpiry !== undefined && refreshTokenExpiry !== null) {
      sessionStorage.setItem(
        "lqsRefreshTokenExpiry",
...
const generateNewAccessToken = async () => {
  try {
    ...
    const expiresIn = data?.expires_in * 1000; // Convert to milliseconds - API returning in seconds - 1800

    tokenExpiry = new Date().getTime() + expiresIn;
    refreshTokenExpiry = 0;

    // Store the tokens and expiry times
    saveTokensToSession(
      accessToken,
      tokenExpiry,
      refreshToken,
...
const getAccessToken = async () => {
  try {
    // Retrieve tokens and expiry times from sessionStorage
    if (typeof window !== "undefined") {
      accessToken = sessionStorage.getItem("lqsAccessToken");
      tokenExpiry = Number(sessionStorage.getItem("lqsTokenExpiry"));
      refreshToken = sessionStorage.getItem("lqsRefreshToken");
      refreshTokenExpiry = Number(
        sessionStorage.getItem("lqsRefreshTokenExpiry")
      );
    }
    ...
    // Step 3: If access token is expired, check if refresh token is valid
    if (accessToken && refreshToken && currentTime > tokenExpiry) {
      if (refreshTokenExpiry === 0 || currentTime < refreshTokenExpiry) {
        const params = new URLSearchParams();
        ...
        try {
          ...
          const expiresIn = data?.expires_in * 1000; // Access token expiry in milliseconds
          const refreshExpiresIn = data?.expires_in * 1000; // Default to 30 minutes

          tokenExpiry = currentTime + expiresIn;
          refreshTokenExpiry = currentTime + refreshExpiresIn;
    ...
    // Step 4: If refresh token is expired, generate new access token via client credentials flow
    return await generateNewAccessToken();
  } catch (error) {
    console.error(
...
const pushToNewLQS = async data => {
  ...
  // Get the access token (this will handle fetching or refreshing if needed)
  const accessToken = await getAccessToken();

  // Check if accessToken was retrieved successfully
  if (!accessToken) {
